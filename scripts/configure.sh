...
sed -i "s/app/${PROJECT}/g" build/template-go/Dockerfile

echo "Creating main service from template app..."
sed -i "s/template-go/${PROJECT}/g" cmd/app/main.go
sed -i "s/cmd\/app/cmd\/${PROJECT}/g" cmd/app/main.go
...
sed -i "s/template-go/${PROJECT}/g" .github/workflows/build-and-push.yml
